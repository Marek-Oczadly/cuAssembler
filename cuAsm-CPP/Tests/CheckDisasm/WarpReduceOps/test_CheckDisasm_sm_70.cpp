...
/**
 * @brief Round-trips the WarpReduceOps kernel through nvcc -> disassemble -> reassemble for
 ...
 *        sm_70 is older than sm_80, since this kernel's instructions require
 *        sm_80 or newer.
 * @return 0 if the reassembled cubin matches the original (or the check was skipped), 1
 *         otherwise.
 **/
int main() {
    return CuAsm::Test::runCheckDisasm("WarpReduceOps", "sm_70", 80) ? 0 : 1;
}
